...
public class DungeonLayout
{
    ...
    private void WalkRooms(RoomType a, RoomType b){
        ...
        while (x != tx || y != ty){
            ...
            for (int i =0; i < dis; i++){
                ...
                if(GetLilRoom(x,y) == 0) SetLilRoom(x, y, 1);
        ...
        for(int x = 0; x < lilrooms.GetLength(0); x++){
            for (int y = 0; y < lilrooms.GetLength(1); y++){
                ...
                if(GetBigRoom(xb,yb) < val){
                    SetBigRoom(xb, yb, val); //Set bigroom type to lilroom type if the lilroom is higher priority
                } 
    ...
    public int[,] GetRooms(){
        ...
        if(bigrooms == null)
        {
            Debug.LogError("Dungeon Layout: must call Generate() before calling GetRooms()");
            return new int[0, 0];
        }
