...
public class InstanceCollection3{
...
public class Plant : MonoBehaviour
{
    ...
    public void Build()
    {
        ...
        iterations = Random.Range(4, 20);
        turnDegrees = Random.Range(0, 46);
        twistDegrees = Random.Range(-90, 46);
        alignWithParent = Random.Range(0, 0.5f);
        ...
        Grow(instances, Vector3.zero, Quaternion.identity, new Vector3(.025f, 0.1f, .025f) , iterations);
    ...
    void Grow(InstanceCollection2 instances, Vector3 pos, Quaternion rot, Vector3 scale, int max, int num = 0, float nodeSpin = 0) {
        ...
        if ((pos - endpoint).sqrMagnitude < .001f) return; //too small, stop recursion
