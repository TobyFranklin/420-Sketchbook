...
public class City : MonoBehaviour
{
    ...
    private void GenerateCity()
    {
        for (int x = 0; x < cityWidth; x++)
        {
            ...
            {
                if (city[x, z] == 0) MakeBlock(x, z);

            }
        }

    }
    private void MakeBlock(int xPos, int zPos) {

        int blockType = Random.Range(1, 3);

        for (int x = xPos; x < xPos + blockWidth + 1; x++){
            for (int z = zPos; z < zPos + blockLength + 1; z++){

                if(x > cityWidth || z > cityLength) continue;
                SetType(x, z, blockType);

                if (x == xPos + blockWidth || z == zPos + blockLength) SetType(x, z, 5);
    ...
    {
        if (xPos < cityWidth && zPos < cityLength) city[xPos, zPos] = type;
    }

    private int GetType(int xPos, int zPos)
    {
        if (xPos < cityWidth && zPos < cityLength) return city[xPos, zPos];
        return -1;
    }
