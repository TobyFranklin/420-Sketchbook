...
[RequireComponent(typeof(MeshFilter))] // tells Unity to require a MeshFilter on any object with a Mesh Controller
...
public class ChunkMeshController : MonoBehaviour
{
    ...
    [Range(0, 1)]
    public float densityThreshold = .5f;
    ...
    void BuildMesh(){
        ...
        // sample our "noise field" to determine solidity (whether each cube will be shown so that we know if we have to render it)
    ...
    void AddCube(Vector3 position,
       ...
       List<Vector2> uvs){
        ...
        if ((sides & 01) > 0) AddQuad(position + new Vector3(0, +0.5f, 0), Quaternion.identity, verts, tris, norms, uvs);
        ...
        if ((sides & 8) > 0) AddQuad(position + new Vector3(-0.5f, 0, 0), Quaternion.Euler(0, 0, +90), verts, tris, norms, uvs);
