class Dungeon{
  ...
  void generate(){
    ...
    for(int x = 0; x < bigrooms.length; x++){
     for(int y = 0; y < bigrooms[x].length; y++){
       ...
       neighbors[0] = getBigRoom(x-1,y-1);//above
       neighbors[1] = getBigRoom(x,y-1);
       neighbors[2] = getBigRoom(x+1,y-1);
       neighbors[3] = getBigRoom(x-11,y+1);//below
       neighbors[4] = getBigRoom(x,y+1);
       neighbors[5] = getBigRoom(x+1,y+1);
       neighbors[6] = getBigRoom(x+1,y);//right
       neighbors[7] = getBigRoom(x-1,y);//left
  ...
  void walkRooms(int type1, int type2){
   ...
   int halfH = rooms[0].length/2;
    ...
    if(x < halfW) tx += halfW; // if starting point on left, move end point to right side
    if(y < halfH) ty += halfH; // if starting point is on top, move endpoint down side
    ...
    setRoom(tx, ty, type2);
        ...
        while(x != tx || y != ty){
          ...
          int dy = ty - y;
            ...
            if(abs(dx) < abs(dy)){ // are we closer to the tx or the ty
              dir = (dy < 0) ? 0 : 1;
            }
            else dir = (dx < 0) ? 3 : 2;
